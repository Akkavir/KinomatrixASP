...
function displayMovie(movie) {
    document.body.innerHTML = `
        ...
        <div class="right-column">
            ...
            <div id="star-rating" class="star-rating">${generateStars(movie.imdbRating)}</div>
            <div id="average-rating">${(calculateFullRating(movie) / 10).toFixed(1)}/10</div>
...
function calculateFullRating(data) {
    let validRatingCount = 0;
    let totalRating = 0;

    // Process IMDB rating (scale is 0-10, convert to 0-100)
    if (data.imdbRating && !isNaN(Number(data.imdbRating))) {
        totalRating += Number(data.imdbRating) * 10;
        validRatingCount++;
    }

    // Process Metascore rating (already on 0-100 scale)
    if (data.Metascore && !isNaN(Number(data.Metascore))) {
        totalRating += Number(data.Metascore);
        validRatingCount++;
    }

    // Process Rotten Tomatoes rating (format: "XX%")
    const rottenTomatoesRating = data.Ratings &&
        Array.isArray(data.Ratings) &&
        data.Ratings.length > 1 &&
        data.Ratings[1]?.Value;

    if (rottenTomatoesRating) {
        const parsedRating = parseFloat(rottenTomatoesRating.replace('%', ''));
    ...
    const averageRating = Math.round(totalRating / validRatingCount);

    // Optional logging
    console.log(`Average rating: ${averageRating} (from ${validRatingCount} sources)`);

    return averageRating;
}


function generateRatings(data) {
    let ratings = data.Ratings.map((r, i) => i === 0 ? `IMDB: ${r.Value}` : `${r.Source}: ${r.Value}`).join(" | ");
    return ratings;
}

function generateStars(rating) {
    const stars = Math.round(parseFloat(rating)); // IMDb rates out of 10, we want 5-star format
    return '★'.repeat(stars) + '☆'.repeat(10 - stars);
}

function formatGenres(genreString) {
    return genreString.split(', ').map(genre => `<span class="genre">${genre}</span>`).join(' ');
}
